...
import type { Student, Course, ChosenCourse } from '../types';
import { getCourses, getChosenCourses, getStudentDetails, updateCourseScore } from '../utils/Fetches';
...
export const StudentCourseManager: React.FC<StudentCourseManagerProps> = ({
  ...
  const [editingScore, setEditingScore] = useState<{ courseId: string; score: string } | null>(null);
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const chosenCourses = await getChosenCourses(student.id);
        ...
        const availableCourses: Course[] = courses.filter((course: Course) => 
          ...
          course.grade <= data.averageScore
        );
                    ...
                    >
                          ...
                          ID: {course.id} | Credit: {course.credit} | Score: {
                            editingScore?.courseId === course.id ? (
                              <input
                                ...
                                onChange={(e) => setEditingScore({ courseId: course.id, score: e.target.value })}
                              ...
                              <span className="cursor-pointer" onClick={() => setEditingScore({ courseId: course.id, score: course.score?.toString() || '' })}>
                                {course.score ? course.score : 'Not graded yet'}
                              </span>
                            )
                          }
                        </p>
                      ...
                      <div className="flex space-x-2">
                        {user?.role == "teacher" && (<button
                          onClick={() => setEditingScore({ courseId: course.id, score: course.score?.toString() || '' })}
